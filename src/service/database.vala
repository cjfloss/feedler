...
*/	
	public void insert_opml (GLib.List<Folder?> folders, GLib.List<Channel?> channels)
	{
		try
        {
			transaction = db.begin_transaction ();
			...
			{
				query = transaction.prepare ("INSERT INTO `channels` (`title`, `source`, `link`, `folder`, `type`) VALUES (:title, :source, :homepage, :folder, :type);");
	...
	public void insert_items (GLib.List<Item?> items, int channel_id)
	{
        try
        {
			transaction = db.begin_transaction ();
	...
	public void insert_subscription (ref Channel channel)
	{
        try
        {
			transaction = db.begin_transaction ();
			query = transaction.prepare ("INSERT INTO `channels` (`title`, `source`, `link`, `folder`, `type`) VALUES (:title, :source, :link, :folder, :type);");
			query.set_string (":title", channel.title);
			query.set_string (":source", channel.source);
			query.set_string (":link", channel.link);
			query.set_int (":folder", channel.folder);
...
/*	
	...
	public unowned GLib.List<Folder?> select_folders ()
	{
        try
        {
			int count = 0;
	...
	public unowned GLib.List<Channel?> select_channels ()
	{
        try
        {
			int count = 0;
