        ...
        while (outline != null) {
            if (outline.name == "outline") {
                ...
                if (type == "rss" || type == "atom") {
                    ...
                    warning ("Following type is currently not supported: " + type + ".");
                    continue;
                }
            }
...
public class BackendXml : Backend {
    ...
    public override void import (string uri) {
        ...
        try {
            ...
            warning ("Cannot run import threads.");
        }
    }
    ...
    private void update_func (Soup.Session session, Soup.Message message) {
        ...
        if (xml != null && this.refresh (xml, out channel)) {
            ...
            channel = Serializer.Channel.no_data ();
        }
    ...
    private bool is_valid (Xml.Doc ? doc) {
        if (doc == null) {
            warning ("Failed to read the source data.");
            return false;
        }
        ...
        if (root == null) {
            warning ("Source data is empty.");
            return false;
        }

        if (root.name != "rss" && root.name != "feed" && root.name != "opml") {
            warning ("Undefined type of data.");
            return false;
        }
